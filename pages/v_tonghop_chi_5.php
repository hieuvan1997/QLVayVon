...
<?php
$sql = "SELECT* FROM v_tonghop_chi_5";
...
if (isset($_POST['btnExport'])) {
    ...
    $rowCount = 1;
    $sheet->setCellValue('A' . $rowCount, 'Tên công đoàn cơ sở');
    $sheet->setCellValue('B' . $rowCount, 'Tổng số tiền vay');
    $sheet->setCellValue('C' . $rowCount, 'Số tiền lãi hàng tháng');
    $sheet->setCellValue('D' . $rowCount, 'Số tháng thu tiền lãi');
    $sheet->setCellValue('E' . $rowCount, 'Số tiền nhận được 5% lãi hàng tháng');
    $sql1 = "SELECT* FROM v_tonghop_chi_5";
    $query1 = mysqli_query($con, $sql1);
    while ($row = mysqli_fetch_array($query1)) {
        $rowCount++;

        $sheet->setCellValue('A' . $rowCount, $row['ten_cdcs']);
        $sheet->setCellValue('B' . $rowCount, $row['tongsovonvay']);
        $sheet->setCellValue('C' . $rowCount, $row['tongsotienlaithuduoc_thang']);
        $sheet->setCellValue('D' . $rowCount, $row['tongsothangthutienlai']);
...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 style="text-align: center" class="page-header">Bảng tổng hợp chi 5% lãi</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
                            ...
                            <table width="100%" class="table table-striped table-bordered table-hover" id="dataTables-example">
                                <thead>
                                    <tr>
                                        ...
                                        <th>Số tiền lãi hàng tháng</th>
                                        <th>Số tháng thu tiền lãi</th>
                                        <th>Số tiền nhận được 5% lãi hàng tháng</th>
                                ...
                                <tbody>
                                    ...
                                    while ($data = mysqli_fetch_array($query)) {
                                        ...
                                        <tr class="odd gradeX">
                                            ...
                                            <td><?php echo $data['tongsotienlaithuduoc_thang']; ?></td>
                                            <td><?php echo $data['tongsothangthutienlai']; ?></td>
