...
if (isset($_POST['btnExport'])) {
    ...
    $sheet->setCellValue('B' . $rowCount, 'Tên đơn công đoàn cấp trên');
    $sheet->setCellValue('C' . $rowCount, 'Tên công đoàn cơ sở');
    $sheet->setCellValue('D' . $rowCount, 'Tên khách hàng');
    $sheet->setCellValue('E' . $rowCount, 'Mức vay được duyệt');
    $sheet->setCellValue('F' . $rowCount, 'Đã trả vốn gốc');
    $sheet->setCellValue('G' . $rowCount, 'Phải thu vốn gốc');
    $sheet->setCellValue('G' . $rowCount, 'Lãi tháng');
    $sql1 = "SELECT* FROM v_bangtonghop";
    $query1 = mysqli_query($con, $sql1);
    while ($row = mysqli_fetch_array($query1)) {
        $rowCount++;
        $sheet->setCellValue('A' . $rowCount, $row['ngayvay']);
        $sheet->setCellValue('B' . $rowCount, $row['ten_cdct']);
        $sheet->setCellValue('C' . $rowCount, $row['ten_cdcs']);
        $sheet->setCellValue('D' . $rowCount, $row['ten_khachhang']);
        $sheet->setCellValue('E' . $rowCount, $row['mucvayduocduyet']);
        $sheet->setCellValue('F' . $rowCount, $row['datravongoc']);
        $sheet->setCellValue('G' . $rowCount, $row['travongochangthang']);
...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 class="page-header" style="text-align: center">Bảng tổng hợp tên người vay, tên các CDCS, CDCT và số tiền vay, đã trả, còn phải thu vốn gốc, lãi của tháng</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
