...
<?php
$sql = "SELECT* FROM v_tinhlai_theo_nguoi";
...
if (isset($_POST['btnExport'])) {
    ...
    $sheet->setCellValue('B' . $rowCount, 'Số tiền vay');
    $sheet->setCellValue('C' . $rowCount, 'Thời gian vay');
    $sheet->setCellValue('D' . $rowCount, 'Đã trả vốn gốc');
    $sheet->setCellValue('E' . $rowCount, 'Trả vốn gốc hàng tháng');
    $sheet->setCellValue('F' . $rowCount, 'Lãi vay theo tháng');
    $sheet->setCellValue('G' . $rowCount, 'Tiết kiệm bắt buộc');
    $sheet->setCellValue('H' . $rowCount, 'Tổng số tiền phải nộp');
    $sql1 = "SELECT* FROM v_tinhlai_theo_nguoi";
    $query1 = mysqli_query($con, $sql1);
    while ($row = mysqli_fetch_array($query1)) {
        $rowCount++;
        $sheet->setCellValue('A' . $rowCount, $row['ten_khachhang']);
        $sheet->setCellValue('B' . $rowCount, $row['mucvayduocduyet']);
        $sheet->setCellValue('C' . $rowCount, $row['thoigianvay']);
        $sheet->setCellValue('D' . $rowCount, $row['datravongoc']);
        $sheet->setCellValue('E' . $rowCount, $row['travongochangthang']);
        $sheet->setCellValue('F' . $rowCount, $row['layvaytheothang']);
        $sheet->setCellValue('G' . $rowCount, $row['tietkiembatbuoc']);
        $sheet->setCellValue('H' . $rowCount, $row['tongsotienphainop']);
    }
...
<body>
    ...
    <div id="wrapper">
        ...
        <div id="page-wrapper">
            <div class="row">
                <div class="col-lg-12">
                    <h1 style="text-align: center" class="page-header">Bảng tính lãi từng tháng theo kỳ của từng người thuộc CĐCS</h1>
                </div>
                <!-- /.col-lg-12 -->
            </div>
            <!-- /.row -->
